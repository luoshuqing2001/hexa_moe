...
class MoE_Cascaded(torch.nn.Module):
    ...
    def __init__(
        ...
        self.local_dim_list_all, self.ids_list_global, self.local_dim_bound_list = self.dim_split(self.hidden_size_list_moe)
    ...
    def dim_split(self, hidden_size_list):
        tmp_dims_list_global = [[] for _ in range(self.world_size)]
        tmp_ids_list_global = [[] for _ in range(self.world_size)]
        tmp_dims_bound_list = []

        for hidden_size in hidden_size_list:
            tmp_dims_list = [int(item * hidden_size) for item in self.mlp_proportion]
            delta = hidden_size - sum(tmp_dims_list)
            this_idx = 0
            while (sum(tmp_dims_list) < hidden_size):
                tmp_dims_list[this_idx] += 1
                this_idx = (this_idx + 1) % self.world_size

            start_idx = 0
            for i in range(self.world_size):
                tmp_dims_list_global[i].append(tmp_dims_list[i])
                tmp_ids_list_global[i].append([*range(start_idx, start_idx + tmp_dims_list[i])])
                start_idx = start_idx + tmp_dims_list[i]

            tmp_dims_bound_list.append(max(tmp_dims_list))
    ...
    def save_ckpt(self, dst_moe, dst_rank=0):
        # dst_moe is an instance of MoE_Buffer class, which is always kept on CPU
        for depth_idx in range(self.total_depth_moe):
            self.dispatch_params.batched_fc1_w_cache = [torch.empty(self.num_global_experts, \
                self.local_dim_list_all[i][depth_idx], self.input_dim_list_moe[depth_idx], \
                device=self.batched_fc1_w.device, dtype=self.batched_fc1_w.dtype) for i in range(self.world_size)]
            comm_tensor = self.batched_fc1_w[depth_idx].data[:, :self.local_dim_list[depth_idx], :].contiguous()
            if comm_tensor.shape[1] < self.local_dim_bound:
                padding_1 = torch.zeros(self.num_global_experts, self.local_dim_bound - comm_tensor.shape[1], comm_tensor.shape[2]).cuda()
                comm_tensor = torch.cat([comm_tensor, padding_1], dim=1)
            if comm_tensor.shape[2] < self.input_dim_bound:
                padding_2 = torch.zeros(self.num_global_experts, self.local_dim_bound, self.input_dim_bound - comm_tensor.shape[2]).cuda()
                comm_tensor = torch.cat([comm_tensor, padding_2], dim=2)
            dist.all_gather(self.dispatch_params.batched_fc1_w_cache, comm_tensor, group=dist.group.WORLD, async_op=False)

            self.dispatch_params.batched_fc2_w_cache = [torch.empty(self.num_global_experts, \
                self.local_dim_list_all[i][depth_idx], self.output_dim_list_moe[depth_idx], \
                device=self.batched_fc2_w.device, dtype=self.batched_fc2_w.dtype) for i in range(self.world_size)]
            comm_tensor = self.batched_fc2_w[depth_idx].data[:, :self.local_dim_list[depth_idx], :].contiguous()
            if comm_tensor.shape[1] < self.local_dim_bound:
                padding_1 = torch.zeros(self.num_global_experts, self.local_dim_bound - comm_tensor.shape[1], comm_tensor.shape[2]).cuda()
                comm_tensor = torch.cat([comm_tensor, padding_1], dim=1)
            if comm_tensor.shape[2] < self.output_dim_bound:
                padding_2 = torch.zeros(self.num_global_experts, self.local_dim_bound, self.output_dim_bound - comm_tensor.shape[2]).cuda()
                comm_tensor = torch.cat([comm_tensor, padding_2], dim=2)
            dist.all_gather(self.dispatch_params.batched_fc2_w_cache, comm_tensor, group=dist.group.WORLD, async_op=False)

            if self.has_fc1_bias:
                self.dispatch_params.batched_fc1_bias_cache = [torch.empty(self.num_global_experts, self.local_dim_list_all[i][depth_idx], \
                    device=self.batched_fc1_bias.device, dtype=self.batched_fc1_bias.dtype) for i in range(self.world_size)]
                comm_tensor = self.batched_fc1_bias[depth_idx].data[:, :self.local_dim_list[depth_idx], :].contiguous()
                if comm_tensor.shape[1] < self.local_dim_bound:
                    padding_1 = torch.zeros(self.num_global_experts, self.local_dim_bound - comm_tensor.shape[1]).cuda()
                    comm_tensor = torch.cat([comm_tensor, padding_1], dim=1)
                dist.all_gather(self.dispatch_params.batched_fc1_bias_cache, comm_tensor, group=dist.group.WORLD, async_op=False)

            if self.local_rank == dst_rank:
                for i in range(self.world_size):
                    dst_moe.batched_fc1_w[depth_idx].data[:, self.ids_list_global[i][depth_idx], :] = \
                        self.dispatch_params.batched_fc1_w_cache[i][:, \
                            :len(self.ids_list_global[i][depth_idx]), :self.input_dim_list_moe[depth_idx]].cpu()
                    dst_moe.batched_fc2_w[depth_idx].data[:, self.ids_list_global[i][depth_idx], :] = \
                        self.dispatch_params.batched_fc2_w_cache[i][:, \
                            :len(self.ids_list_global[i][depth_idx]), :self.output_dim_list_moe[depth_idx]].cpu()
                    if self.has_fc1_bias:
                        dst_moe.batched_fc1_bias[depth_idx].data[:, self.ids_list_global[i][depth_idx]] = \
                            self.dispatch_params.batched_fc1_bias_cache[i][:, \
                                :len(self.ids_list_global[i][depth_idx])].cpu()

                if self.has_fc2_bias:
                    dst_moe.batched_fc2_bias[depth_idx].data = self.batched_fc2_bias[depth_idx].data.cpu()
    ...
    def forward(self, depth_idx: int, input: Tensor, gate_index=0, top_k=None, a2a_ffn_overlap_degree=None, reserve_dims=1):       
        ...
        if not moe_flag:
            y = MLP_Implement.apply(self.dispatch_params, x, \
                    ...
                    self.mlp_fc1_b[depth_idx_mlp], self.mlp_fc2_b[depth_idx_mlp]).to(x.dtype)
        ...
        y = FusedMoE_Shared_CudaCore.apply(self.dispatch_params, data, \
                param_1_w, param_2_w, param_1_b, param_2_b).to(x.dtype)
