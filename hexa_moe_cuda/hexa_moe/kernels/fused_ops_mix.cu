...
__global__ void fused_grad_bias_kernel_vectorized_mix(
    float *j_b,
    float *j_x,
    float *j_w,
    __half *j_y,
            ...
            for (int j = 0; j < 2; j++) {
                if ((tok_idx_0 < 0) || (tok_idx_0 >= num_tokens)) {
                    ...
                    if (out_dims_base + 3 < num_out_dims) {
                        ...
                        Half4 tmp_half4 = make_Half4_from_half(tmp_half_x, tmp_half_y, tmp_half_z, __float2half(0.0f));
                        reinterpret_cast<Half4 *>(&smem_tok[ThreadTok * BLOCK_SIZE + ThreadOut * threads_per_chunk + j * 4])[0] = tmp_half4;
                    } else if (out_dims_base + 1 < num_out_dims) {
                        __half tmp_half_x = j_y[output_offset + tok_idx_0 * num_out_dims + out_dims_base];
                        __half tmp_half_y = j_y[output_offset + tok_idx_0 * num_out_dims + out_dims_base + 1];
                        Half4 tmp_half4 = make_Half4_from_half(tmp_half_x, tmp_half_y, __float2half(0.0f), __float2half(0.0f));
                        reinterpret_cast<Half4 *>(&smem_tok[ThreadTok * BLOCK_SIZE + ThreadOut * threads_per_chunk + j * 4])[0] = tmp_half4;
                    } else if (out_dims_base < num_out_dims) {
                        __half tmp_half_x = j_y[output_offset + tok_idx_0 * num_out_dims + out_dims_base];
                        Half4 tmp_half4 = make_Half4_from_half(tmp_half_x, __float2half(0.0f), __float2half(0.0f), __float2half(0.0f));
                        reinterpret_cast<Half4 *>(&smem_tok[ThreadTok * BLOCK_SIZE + ThreadOut * threads_per_chunk + j * 4])[0] = tmp_half4;
                    } else {
                        reinterpret_cast<Half4 *>(&smem_tok[ThreadTok * BLOCK_SIZE + ThreadOut * threads_per_chunk + j * 4])[0] =\
                            reinterpret_cast<Half4 *>(zero_pad_half4)[0];
...
void launch_fused_grad_bias_mix(
    ...
    */
    ...
    fused_grad_bias_kernel_vectorized_mix<<<Block, Grid>>>(
        j_b, j_x, j_w, j_y, w, x, \
...
__global__ void fused_grad_no_bias_kernel_vectorized_mix(
    float *j_x,
    float *j_w,
    __half *j_y,
    float *w,
...
void launch_fused_grad_no_bias_mix(
    ...
    */
    ...
    fused_grad_no_bias_kernel_vectorized_mix<<<Block, Grid>>>(
        j_x, j_w, j_y, w, x, \
        token_idx_list, expert_idx_list, expert_start_list, \
        m2s, grid_dim_z_1, num_tokens, num_in_dims, num_out_dims, num_routings
    );
