...
__global__ void esmm_shared_tensor_kernel_vectorized_accum_full(
    float *result,
    float *tokens,
    float *weights,
    float *bias,
        ...
        for (int j = 0; j < 2; j++) {
            if ((tok_idx < 0) || (tok_idx >= num_tokens)) {
                ...
                if (in_dims_base + 3 < num_in_dims) {
                    float4 tmp_float4 = reinterpret_cast<float4 *>(&tokens[input_offset + tok_idx * num_in_dims + in_dims_base])[0];
                    ...
                    float tmp_float_x = tokens[input_offset + tok_idx * num_in_dims + in_dims_base];
                    float tmp_float_y = tokens[input_offset + tok_idx * num_in_dims + in_dims_base + 1];
                    float tmp_float_z = tokens[input_offset + tok_idx * num_in_dims + in_dims_base + 2];
...
void launch_esmm_accum_shared_tensor_full(
    ...
    esmm_shared_tensor_kernel_vectorized_accum_full<<<Grid, Block>>>(
