...
__global__ void esmm_shared_tensor_kernel_vectorized_accum_half(
    float *result,
    __half *tokens,
    __half *weights,
    __half *bias,
    ...
    if ((out_dims_base < num_out_dims) && (ThreadOut == 0)) {
        if ((bias != nullptr) && (exp_idx >= 0) && (exp_idx < num_routings)) {
            smem_bias[ThreadTile * BLOCK_SIZE + ThreadTok] = __half2float(bias[exp_idx * num_out_dims + out_dims_base]);
        } else {
        ...
        for (int j = 0; j < 2; j++) {
            if ((tok_idx < 0) || (tok_idx >= num_tokens)) {
                ...
                if (in_dims_base + 3 < num_in_dims) {
                    ...
                    Half4 tmp_half4 = make_Half4(tmp_float_x, tmp_float_y, tmp_float_z, __float2half(0.0f));
                    reinterpret_cast<Half4 *>(&smem_tok[ThreadTok * BLOCK_SIZE + ThreadOut * threads_per_chunk + j * 4])[0] = tmp_half4;
                } else if (in_dims_base + 1 < num_in_dims) {
                    __half tmp_float_x = tokens[input_offset + tok_idx * num_in_dims + in_dims_base];
                    __half tmp_float_y = tokens[input_offset + tok_idx * num_in_dims + in_dims_base + 1];
                    Half4 tmp_half4 = make_Half4(tmp_float_x, tmp_float_y, __float2half(0.0f), __float2half(0.0f));
                    reinterpret_cast<Half4 *>(&smem_tok[ThreadTok * BLOCK_SIZE + ThreadOut * threads_per_chunk + j * 4])[0] = tmp_half4;
                } else if (in_dims_base < num_in_dims) {
                    __half tmp_float_x = tokens[input_offset + tok_idx * num_in_dims + in_dims_base];
                    Half4 tmp_half4 = make_Half4(tmp_float_x, __float2half(0.0f), __float2half(0.0f), __float2half(0.0f));
                    reinterpret_cast<Half4 *>(&smem_tok[ThreadTok * BLOCK_SIZE + ThreadOut * threads_per_chunk + j * 4])[0] = tmp_half4;
                } else {
                    reinterpret_cast<Half4 *>(&smem_tok[ThreadTok * BLOCK_SIZE + ThreadOut * threads_per_chunk + j * 4])[0] =\
                        reinterpret_cast<Half4 *>(zero_pad_half4)[0];
            ...
            if ((exp_idx < 0) || (exp_idx >= num_routings)) {
                ...
                if (in_dims_base >= num_in_dims) {
                    ...
                    if (out_dims_base + 3 < num_out_dims) {
                        Half4 tmp_half4 = reinterpret_cast<Half4 *>(&weights[exp_idx * num_in_dims * num_out_dims + in_dims_base * num_out_dims + out_dims_base])[0];
                        ...
                        __half tmp_float_x = weights[exp_idx * num_in_dims * num_out_dims + in_dims_base * num_out_dims + out_dims_base];
                        __half tmp_float_y = weights[exp_idx * num_in_dims * num_out_dims + in_dims_base * num_out_dims + out_dims_base + 1];
                        __half tmp_float_z = weights[exp_idx * num_in_dims * num_out_dims + in_dims_base * num_out_dims + out_dims_base + 2];
                        Half4 tmp_half4 = make_Half4(tmp_float_x, tmp_float_y, tmp_float_z, __float2half(0.0f));
                        reinterpret_cast<Half4 *>(&smem_op[ThreadTok * TILE * BLOCK_SIZE + ThreadTile * BLOCK_SIZE + ThreadOut * threads_per_chunk + j * 4])[0] = tmp_half4;
...
void launch_esmm_accum_shared_tensor_half(
    ...
    esmm_shared_tensor_kernel_vectorized_accum_half<<<Grid, Block>>>(
