...
__forceinline__ __host__ __device__ Half4 make_Half4(const __half x, const __half y, const __half z, __half w)
{
    Half4 h4;
...
__global__ void fused_grad_bias_kernel_vectorized_half(
    float *j_b,
    float *j_x,
    float *j_w,
    __half *j_y,
            ...
            for (int j = 0; j < 2; j++) {
                if ((tok_idx_0 < 0) || (tok_idx_0 >= num_tokens)) {
                    ...
                    if (out_dims_base + 3 < num_out_dims) {
                        Half4 tmp_half4 = reinterpret_cast<Half4 *>(&j_y[output_offset + tok_idx_0 * num_out_dims + out_dims_base])[0];
                        ...
                        __half tmp_half_x = j_y[output_offset + tok_idx_0 * num_out_dims + out_dims_base];
                        __half tmp_half_y = j_y[output_offset + tok_idx_0 * num_out_dims + out_dims_base + 1];
                        __half tmp_half_z = j_y[output_offset + tok_idx_0 * num_out_dims + out_dims_base + 2];
                        Half4 tmp_half4 = make_Half4(tmp_half_x, tmp_half_y, tmp_half_z, __float2half(0.0f));
                        reinterpret_cast<Half4 *>(&smem_tok[ThreadTok * BLOCK_SIZE + ThreadOut * threads_per_chunk + j * 4])[0] = tmp_half4;
                    } else if (out_dims_base + 1 < num_out_dims) {
                        __half tmp_half_x = j_y[output_offset + tok_idx_0 * num_out_dims + out_dims_base];
                        __half tmp_half_y = j_y[output_offset + tok_idx_0 * num_out_dims + out_dims_base + 1];
                        Half4 tmp_half4 = make_Half4(tmp_half_x, tmp_half_y, __float2half(0.0f), __float2half(0.0f));
                        reinterpret_cast<Half4 *>(&smem_tok[ThreadTok * BLOCK_SIZE + ThreadOut * threads_per_chunk + j * 4])[0] = tmp_half4;
                    } else if (out_dims_base < num_out_dims) {
                        __half tmp_half_x = j_y[output_offset + tok_idx_0 * num_out_dims + out_dims_base];
                        Half4 tmp_half4 = make_Half4(tmp_half_x, __float2half(0.0f), __float2half(0.0f), __float2half(0.0f));
                        reinterpret_cast<Half4 *>(&smem_tok[ThreadTok * BLOCK_SIZE + ThreadOut * threads_per_chunk + j * 4])[0] = tmp_half4;
                    } else {
                        reinterpret_cast<Half4 *>(&smem_tok[ThreadTok * BLOCK_SIZE + ThreadOut * threads_per_chunk + j * 4])[0] =\
                            reinterpret_cast<Half4 *>(zero_pad_half4)[0];
                ...
                if ((exp_idx_0 < 0) || (exp_idx_0 >= num_routings)) {
                    ...
                    if (out_dims_base >= num_out_dims) {
                        ...
                        if (in_dims_base + 3 < num_in_dims) {
                            Half4 tmp_half4 = reinterpret_cast<Half4 *>(&w[exp_idx_0 * num_out_dims * num_in_dims + out_dims_base * num_in_dims + in_dims_base])[0];
                            ...
                            __half tmp_half_x = w[exp_idx_0 * num_out_dims * num_in_dims + out_dims_base * num_in_dims + in_dims_base];
                            __half tmp_half_y = w[exp_idx_0 * num_out_dims * num_in_dims + out_dims_base * num_in_dims + in_dims_base + 1];
                            __half tmp_half_z = w[exp_idx_0 * num_out_dims * num_in_dims + out_dims_base * num_in_dims + in_dims_base + 2];
                            Half4 tmp_half4 = make_Half4(tmp_half_x, tmp_half_y, tmp_half_z, __float2half(0.0f));
                            reinterpret_cast<Half4 *>(&smem_op[ThreadTok * TILE * BLOCK_SIZE + ThreadTile * BLOCK_SIZE + ThreadOut * threads_per_chunk + j * 4])[0] = tmp_half4;
...
#pragma unroll
            for (int j = 0; j < 2; j++) {
                if ((tok_idx < 0) || (tok_idx >= num_tokens)) {
                    ...
                    if (out_dims_base + 3 < num_out_dims) {
                        Half4 tmp_half4 = reinterpret_cast<Half4 *>(&j_y[output_offset + tok_idx * num_out_dims + out_dims_base])[0];
                        ...
                        __half tmp_half_x = j_y[output_offset + tok_idx * num_out_dims + out_dims_base];
                        __half tmp_half_y = j_y[output_offset + tok_idx * num_out_dims + out_dims_base + 1];
                        __half tmp_half_z = j_y[output_offset + tok_idx * num_out_dims + out_dims_base + 2];
                        smem_tok[(ThreadOut * threads_per_chunk + j * 4) * BLOCK_SIZE + ThreadTok] = tmp_half_x;
                        smem_tok[(ThreadOut * threads_per_chunk + j * 4 + 1) * BLOCK_SIZE + ThreadTok] = tmp_half_y;
                        smem_tok[(ThreadOut * threads_per_chunk + j * 4 + 2) * BLOCK_SIZE + ThreadTok] = tmp_half_z;
                        smem_tok[(ThreadOut * threads_per_chunk + j * 4 + 3) * BLOCK_SIZE + ThreadTok] = __float2half(0.0f);
                    } else if (out_dims_base + 1 < num_out_dims) {
                        __half tmp_half_x = j_y[output_offset + tok_idx * num_out_dims + out_dims_base];
                        __half tmp_half_y = j_y[output_offset + tok_idx * num_out_dims + out_dims_base + 1];
...
#pragma unroll
            for (int j = 0; j < 2; j++) {
                if ((tok_idx < 0) || (tok_idx >= num_tokens)) {
                    ...
                    int in_dims_base = TileIn * TILE * BLOCK_SIZE + ThreadTile * BLOCK_SIZE + ThreadOut * threads_per_chunk + j * 4;
                    if (in_dims_base + 3 < num_in_dims) {
                        Half4 tmp_half4 = reinterpret_cast<Half4 *>(&x[input_offset + tok_idx * num_in_dims + in_dims_base])[0];
                        reinterpret_cast<Half4 *>(&smem_op[ThreadTok * TILE * BLOCK_SIZE + ThreadTile * BLOCK_SIZE + ThreadOut * threads_per_chunk + j * 4])[0] = \
                            tmp_half4;
                    } else if (in_dims_base + 2 < num_in_dims) {
                        __half tmp_half_x = x[input_offset + tok_idx * num_in_dims + in_dims_base];
                        __half tmp_half_y = x[input_offset + tok_idx * num_in_dims + in_dims_base + 1];
                        __half tmp_half_z = x[input_offset + tok_idx * num_in_dims + in_dims_base + 2];
                        Half4 tmp_half4 = make_Half4(tmp_half_x, tmp_half_y, tmp_half_z, __float2half(0.0f));
                        reinterpret_cast<Half4 *>(&smem_op[ThreadTok * TILE * BLOCK_SIZE + ThreadTile * BLOCK_SIZE + ThreadOut * threads_per_chunk + j * 4])[0] = \
                            tmp_half4;
                    } else if (in_dims_base + 1 < num_in_dims) {
        ...
        for (int i = loc_exp_start; i < loc_exp_end; i += BLOCK_SIZE) {
            ...
            for (int j = 0; j < 2; j++) {
                if ((tok_idx < 0) || (tok_idx >= num_tokens)) {
                    ...
                    if (out_dims_base + 3 < num_out_dims) {
                        Half4 tmp_half4 = reinterpret_cast<Half4 *>(&j_y[output_offset + tok_idx * num_out_dims + out_dims_base])[0];
                        float4 tmp_float4 = make_float4(__half2float(tmp_half4.x), __half2float(tmp_half4.y), __half2float(tmp_half4.z), __half2float(tmp_half4.w));
                        reinterpret_cast<float4 *>(&smem_result[ThreadTok * TILE * BLOCK_SIZE + ThreadTile * BLOCK_SIZE + ThreadOut * threads_per_chunk + j * 4])[0] =\
                            tmp_float4;
                    } else if (out_dims_base + 2 < num_out_dims) {
                        __half tmp_half_x = j_y[output_offset + tok_idx * num_out_dims + out_dims_base];
                        __half tmp_half_y = j_y[output_offset + tok_idx * num_out_dims + out_dims_base + 1];
                        __half tmp_half_z = j_y[output_offset + tok_idx * num_out_dims + out_dims_base + 2];
                        float4 tmp_float4 = make_float4(__half2float(tmp_half_x), __half2float(tmp_half_y), __half2float(tmp_half_z), 0.0f);
                        reinterpret_cast<float4 *>(&smem_result[ThreadTok * TILE * BLOCK_SIZE + ThreadTile * BLOCK_SIZE + ThreadOut * threads_per_chunk + j * 4])[0] =\
                            tmp_float4;
                    } else if (out_dims_base + 1 < num_out_dims) {
                        __half tmp_half_x = j_y[output_offset + tok_idx * num_out_dims + out_dims_base];
                        __half tmp_half_y = j_y[output_offset + tok_idx * num_out_dims + out_dims_base + 1];
                        float4 tmp_float4 = make_float4(__half2float(tmp_half_x), __half2float(tmp_half_y), 0.0f, 0.0f);
                        reinterpret_cast<float4 *>(&smem_result[ThreadTok * TILE * BLOCK_SIZE + ThreadTile * BLOCK_SIZE + ThreadOut * threads_per_chunk + j * 4])[0] =\
                            tmp_float4;
                    } else if (out_dims_base < num_out_dims) {
                        __half tmp_half_x = j_y[output_offset + tok_idx * num_out_dims + out_dims_base];
                        float4 tmp_float4 = make_float4(__half2float(tmp_half_x), 0.0f, 0.0f, 0.0f);
                        reinterpret_cast<float4 *>(&smem_result[ThreadTok * TILE * BLOCK_SIZE + ThreadTile * BLOCK_SIZE + ThreadOut * threads_per_chunk + j * 4])[0] =\
                            tmp_float4;
                    } else {
                        reinterpret_cast<float4 *>(&smem_result[ThreadTok * TILE * BLOCK_SIZE + ThreadTile * BLOCK_SIZE + ThreadOut * threads_per_chunk + j * 4])[0] =\
...
void launch_fused_grad_bias_half(
    ...
    */
    ...
    fused_grad_bias_kernel_vectorized_half<<<Block, Grid>>>(
        j_b, j_x, j_w, j_y, w, x, \
...
__global__ void fused_grad_no_bias_kernel_vectorized_half(
    float *j_x,
    float *j_w,
    __half *j_y,
    __half *w,
...
void launch_fused_grad_no_bias_half(
    ...
    */
    ...
    fused_grad_no_bias_kernel_vectorized_half<<<Block, Grid>>>(
        j_x, j_w, j_y, w, x, \
        token_idx_list, expert_idx_list, expert_start_list, \
        m2s, grid_dim_z_1, num_tokens, num_in_dims, num_out_dims, num_routings
    );
