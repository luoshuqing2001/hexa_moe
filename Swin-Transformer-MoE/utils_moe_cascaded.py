...
def load_checkpoint_tmoe(config, model, optimizer, lr_scheduler, loss_scaler, logger):
    ...
    if pretrained_path.endswith(f'.rank{global_rank}'):
        ...
        if os.path.exists(pretrained_path.replace(f'.rank{global_rank}', f'.master')):
            checkpoint_master = torch.load(pretrained_path.replace(f'.rank{global_rank}', f'.master'),
                                           map_location='cpu')
            state_dict = merge_moe_model_state_dict(checkpoint['model_without_moe'], checkpoint_master['model_without_moe'])
        else:
            state_dict = checkpoint['model_without_moe']
...
def save_checkpoint_tmoe(config, epoch, model, max_accuracy, optimizer, lr_scheduler, loss_scaler, logger):
    ...
    save_path = os.path.join(config.OUTPUT, f'ckpt_epoch_{epoch}.pth.rank{dist.get_rank()}')
