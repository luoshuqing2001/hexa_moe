...
# class MoEMlp_Cascaded(nn.Module):
...
#         self._moe_layer = hmoe.moe_layer(
...
class SwinTransformerMoE_Cascaded(nn.Module):
    r""" Swin Transformer
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24], num_global_experts=1,
        ...
        self.drop_path_list = nn.ModuleList()
        ...
        for i_layer in range(self.num_layers):
            ...
            input_reso = (patches_resolution[0] // (2 ** i_layer),
                            patches_resolution[1] // (2 ** i_layer))
            ds = PatchMerging if (i_layer < self.num_layers - 1) else None
            # patch merging layer
            if ds is not None:
                ds = ds(input_reso, dim=int(embed_dim * 2 ** i_layer), norm_layer=norm_layer)
            else:
                ds = None
            self.downsample_list.append(ds)

            for j in range(depths[i_layer]):
                drop_path = dpr[sum(depths[:i_layer]):sum(depths[:i_layer + 1])]
                self.drop_path_list.append(DropPath(drop_path[j]) if drop_path[j] > 0. else nn.Identity())
        ...
        for i in range(len(depths)):
            for _ in range(depths[i]):
                model_dim_list.append(embed_dim * 2 ** i)
        
        # Re-Index the moe_blocks list
        counter_base = 0
        moe_blocks_reindex = []
        for i in range(len(depths)):
            for item in moe_blocks[i]:
                if item >= 0:
                    moe_blocks_reindex.append(counter_base + item)
            counter_base += depths[i]

        mlp_idx_list = []
        for i in range(sum(depths)):
            if i not in moe_blocks_reindex:
                mlp_idx_list.append(i)

        self.cascaded_moe = hmoe.MoE_Cascaded(
                gate_type=_gate_type,
                model_dim_list=model_dim_list,
                moe_idx_list=moe_blocks_reindex,
                mlp_ratio=self.mlp_ratio,
                mlp_proportion=None,
    ...
    def forward_features(self, x):
        ...
        for layer in self.layers:
            for sub_layer in layer.blocks:
                x, shortcut = sub_layer(x)
                x, cur_l_aux = self.cascaded_moe(depth_idx, x)
                x = shortcut + self.drop_path_list[depth_idx](x)
                l_aux = cur_l_aux + l_aux
                depth_idx = depth_idx + 1
            if self.downsample_list[out_idx] is not None:
                x = self.downsample_list[out_idx](x)
            out_idx = out_idx + 1
    ...
    def load_ckpt(self, load_moe, src_rank=0):
        return self.cascaded_moe.load_ckpt(load_moe)

    def save_ckpt(self, dst_moe, dst_rank=0):
        return self.cascaded_moe.save_ckpt(dst_moe)

    def cuda_cache(self):
        return self.cascaded_moe.cuda_cache()
